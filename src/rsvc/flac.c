...
bool rsvc_flac_encode(FILE* src_file, FILE* dst_file, rsvc_encode_options_t options, rsvc_done_t fail) {
    ...
    if (!(FLAC__stream_encoder_set_verify(encoder, true) &&
          ...
          FLAC__stream_encoder_set_channels(encoder, info.channels) &&
          FLAC__stream_encoder_set_bits_per_sample(encoder, info.bits_per_sample) &&
          FLAC__stream_encoder_set_sample_rate(encoder, info.sample_rate) &&
          FLAC__stream_encoder_set_total_samples_estimate(encoder, info.samples_per_channel))) {
...
static write_encode_status_t flac_encode_write(const FLAC__StreamEncoder* encoder,
    ...
    while (nbytes > 0) {
        ssize_t written = write(fileno(u->file), data, nbytes);
        if (written <= 0) {
...
static seek_encode_status_t flac_encode_seek(const FLAC__StreamEncoder* encoder,
    ...
    (void)encoder;
    flac_encode_userdata_t u = (flac_encode_userdata_t)userdata;
    if (lseek(fileno(u->file), absolute_byte_offset, SEEK_SET) < 0) {
...
static tell_encode_status_t flac_encode_tell(const FLAC__StreamEncoder* encoder,
    ...
    (void)encoder;
    flac_encode_userdata_t u = (flac_encode_userdata_t)userdata;
    off_t offset = lseek(fileno(u->file), 0, SEEK_CUR);
    if (offset < 0) {
...
static read_decode_status_t flac_decode_read(const FLAC__StreamDecoder* encoder,
    ...
    void* data = bytes;
    while (true) {
        ssize_t bytes_read = read(fileno(u->read_file), data, *nbytes);
        if (bytes_read > 0) {
            *nbytes = bytes_read;
...
static write_decode_status_t flac_decode_write(const FLAC__StreamDecoder* decoder,
    ...
    while (size > 0) {
        ssize_t written = write(fileno(u->write_file), s16, size);
        if (written <= 0) {
            rsvc_strerrorf(u->fail, __FILE__, __LINE__, NULL);
            u->called_done = true;
            free(s16);
...
static seek_decode_status_t flac_decode_seek(const FLAC__StreamDecoder* decoder,
    ...
    (void)decoder;
    flac_decode_userdata_t u = (flac_decode_userdata_t)userdata;
    if (lseek(fileno(u->read_file), absolute_byte_offset, SEEK_SET) < 0) {
...
static tell_decode_status_t flac_decode_tell(const FLAC__StreamDecoder* decoder,
    ...
    (void)decoder;
    flac_decode_userdata_t u = (flac_decode_userdata_t)userdata;
    off_t offset = lseek(fileno(u->read_file), 0, SEEK_CUR);
    if (offset < 0) {
...
static length_decode_status_t flac_decode_length(const FLAC__StreamDecoder* decoder,
    ...
    flac_decode_userdata_t u = (flac_decode_userdata_t)userdata;
    struct stat st;
    if (fstat(fileno(u->read_file), &st) < 0) {
...
static size_t flac_audio_info_read(void *ptr, size_t size, size_t count, FLAC__IOHandle handle) {
    ...
    if (!rsvc_cread(NULL, u->file, ptr, count, size, &count, &u->size_inout, &u->eof, u->fail)) {
        u->called_fail = true;
        return -1;
    }
    return count;
